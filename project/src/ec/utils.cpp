...
void ECProject::random_n_num_deterministic(int min, int max, int n, std::vector<int> &res, std::mt19937& rng)
{
    my_assert(n <= max - min + 1);
    res.clear();
    
    int total_elements = max - min + 1;
    std::vector<int> pool(total_elements);
    
    std::iota(pool.begin(), pool.end(), min);

    std::shuffle(pool.begin(), pool.end(), rng);

    res.assign(pool.begin(), pool.begin() + n);
}
