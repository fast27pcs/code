          ...
          std::vector<std::vector<HelpRepairPlan>>& help_repairs)
  {
    ...
    for (auto& repair_plan : repair_plans) {
      ...
      */
      ...
      // 1. 建立 [全局Block_ID -> 逻辑分区索引 rk] 的映射表
      // 因为 Stripe 知道初始的图纸，所以 Coordinator 可以反查这个块最初属于第几个逻辑分区
      std::unordered_map<int, int> bid2rk;
      for (int rk = 0; rk < stripe.ec->partition_plan.size(); ++rk) {
          for (int bid : stripe.ec->partition_plan[rk]) {
              bid2rk[bid] = rk;
          }
      }

      // 2. 统计各物理机架 (cid) 的坏块数，并顺手查出它对应的逻辑索引 (rk)
      std::unordered_map<unsigned int, int> failures_cnt;
      std::unordered_map<unsigned int, int> cid2rk;
      for (auto& idx : repair_plan.failure_idxs) {
          unsigned int nid = new_blocks2nodes[idx];
          unsigned int cid = node_table_[nid].map2cluster;
          failures_cnt[cid]++;
          cid2rk[cid] = bid2rk[idx]; // 记录这个 cid 对应底层算法的哪一个 rk
      }
      
      // 3. 选大哥：比较坏块数，平局时严格比较逻辑索引 (rk)！
      unsigned int main_cid = 0;
      int max_cnt_val = 0;
      int min_rk = 0x7FFFFFFF; // 设为一个极大的初始值

      for (auto& kv : failures_cnt) {
          ...
          if (cnt > max_cnt_val) {
              ...
              main_cid = cid;
              min_rk = rk;
          } else if (cnt == max_cnt_val && rk < min_rk) {
              // 【终极护城河】：坏块一样多？那就看底层算法里谁的排位 (rk) 更靠前！
              // 这让物理层彻底放弃了 cid 大小的挣扎，100% 服从底层的意志！
              main_cid = cid;
              min_rk = rk;
          }
      }
        ...
        */
        ...
        if (map2clusters[i] == main_cid) {
          for (auto block_idx : repair_plan.help_blocks[i]) {
            ...
            main_plan.inner_cluster_help_block_ids.push_back(stripe.block_ids[block_idx]);
          }
        }       
      }
