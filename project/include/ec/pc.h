...
namespace ECProject
{
	...
	public:
		...
		void generate_partition() override;
		void partition_flat() override;
		void partition_random() override;
		void partition_optimal() override;
		void partition_ablock();
        void partition_acol();
        void partition_arow();
        void partition_m1col();
        void partition_m2row();
        void partition_tile();
        void partition_diagonal();

		void reorganize_help_blocks_by_rack(std::vector<std::vector<int>>& help_blocks);
		bool repair_ablock(const std::vector<int>& failure_idxs,
                           std::vector<RepairPlan>& plans, bool partial_scheme);
        bool repair_acol(const std::vector<int>& failure_idxs,
                         std::vector<RepairPlan>& plans, bool partial_scheme);
        bool repair_arow(const std::vector<int>& failure_idxs,
                         std::vector<RepairPlan>& plans, bool partial_scheme);
        bool repair_m1col(const std::vector<int>& failure_idxs,
                          std::vector<RepairPlan>& plans, bool partial_scheme);
        bool repair_m2row(const std::vector<int>& failure_idxs,
                          std::vector<RepairPlan>& plans, bool partial_scheme);
        bool repair_tile(const std::vector<int>& failure_idxs,
                         std::vector<RepairPlan>& plans, bool partial_scheme);
        bool repair_diagonal(const std::vector<int>& failure_idxs,
                             std::vector<RepairPlan>& plans, bool partial_scheme);
