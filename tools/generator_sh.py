...
networkcore_address = "192.168.0.249:17550"
networkcore_ip = "192.168.0.249"
...
def generate_cluster_info_dict():
    ...
    for i in range(cluster_number):
        new_cluster = {}
        new_cluster["proxy"] = proxy_ip_list[proxy_idx][0]+":"+str(proxy_ip_list[proxy_idx][1] + 10 * proxy_cnt)
        datanode_list = []
...
def generate_run_proxy_datanode_file():
    ...
    with open(file_name, 'w') as f:
        ...
        f.write("\n")
        for cluster_id in cluster_informtion.keys():
            proxy_uri = cluster_informtion[cluster_id]["proxy"]
            proxy_ip, proxy_port = proxy_uri.split(":")
            f.write("./project/build/run_proxy "+proxy_ip+" "+proxy_port+" "+networkcore_address+"&\n")
        f.write("\n")
        f.write("sleep 3\n")
...
def cluster_generate_sh_for_networkcore(i):
    ...
    with open(file_name, 'w') as f:
        ...
        f.write("\n")
        #f.write("./project/build/run_datanode "+"0.0.0.0"+" "+str(networkcore_port)+"&\n")
        cmd = f"nohup ./project/build/run_datanode 192.168.0.249 {networkcore_port} > /dev/null 2>&1 &\n"
        f.write(cmd)
        f.write("\n")
